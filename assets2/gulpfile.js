...
var tools = {
    ...
    uglofy: require('gulp-uglify-es').default,
}
var path = {
    libs: {
        js: {
    ...
    js: {
        files: [
            buildConf.path.build + '/js/libs.min.js',
    ...
    dist: {
        css:{
            files: [
                buildConf.path.build + '/css/libs.min.css',
                buildConf.path.build + '/css/styles.css',
            ],
            name: 'all.css',
            path: buildConf.path.build + '/css',
        }
...
tools.gulp.task('dist:css', ['libs:css', 'styles'], function() {
    return tools.gulp.src(path.dist.css.files)
        .pipe(tools.concat(path.dist.css.name))
        .pipe(tools.cleanCss({compatibility: 'ie8'}))
        .pipe(tools.stripCssComment({preserve: false}))
        .pipe(tools.gulp.dest(path.dist.css.path))
});
...
tools.gulp.task('build', ['js:libs', 'html', 'fonts', 'img', 'js', 'json', 'dist:css']);
